...
class Pessoa(var nome: String, var dataDeNascimento: Date) : Movimentavel {
    ...
    fun temCarta(): Boolean {
        print(carta.toString())
            if (carta.javaClass.simpleName == Carta().javaClass.simpleName) {
                return true
            }
         PessoaSemCartaException("$nome não tem carta para conduzir o veículo indicado")
        return false
    }

    fun tirarCarta() {
            if (LocalDate.now().year - (dataDeNascimento.toString().split(" ")[5].toInt()) >= 18) {
                this.carta = Carta()
            } else {
                MenorDeIdadeException("Não pode tirar a carta" )
            }
    }
